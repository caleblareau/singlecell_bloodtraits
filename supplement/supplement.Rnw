...
ggplot(data=heritability, aes(x=trait,y=h_obs)) +
  geom_bar(stat="identity",fill="steel blue") + ggtitle("Trait Heritability") + 
  pretty_plot()+
          geom_errorbar(aes(ymin=h_obs-h_obs_se, ymax=h_obs+h_obs_se),
                        width=.2,position=position_dodge(.9),color="black") +
  theme(plot.subtitle = element_text(vjust = 1), 
    plot.caption = element_text(vjust = 1), 
    axis.text.x = element_text(angle = 90)) +labs(x = NULL, y = "Narrow sense heritability")
