...
<<ccnd3Piechart, message = FALSE, warning = FALSE, echo = FALSE, fig.height=5, fig.align='center', fig.pos='H', fig.width=8, fig.cap="Haplotypes frequencies of the CCND3 variants rs2408955 and rs112233623 in the UKBB Population. Haplotype labels are formatted as rs2408955/rs112233623.", eval = TRUE, cache = FALSE, results="hide">>=

### Moderating alleles
trait <- "RBC_COUNT"
unit <- "10^6 cells/uL"
rs_alleles <- readRDS("../data/examples/CCND3.haplotypes.rds")
...
plot_ly(proportions, labels = ~Genotype, values = ~Freq, type = 'pie',textposition = 'outside',textinfo = 'label+percent',width = 600, height = 700) %>%
  layout(xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE), autosize = F,  margin = m)
...
<<ccnd3Boxplots, message = FALSE, warning = FALSE, echo = FALSE, fig.height=5, fig.align='center', fig.pos='H', fig.width=8, fig.cap="Moderating effects of CCND3 haplotypes on red blood cell count.", eval = TRUE, cache = FALSE, results="hide">>=
...
# Set y-axis bounds
limits <- c(as.numeric(quantile(rs_alleles_onlyabsolutes[,trait],0.001)),
            as.numeric(quantile(rs_alleles_onlyabsolutes[,trait],0.999)))
...
ggplot(subset(rs_alleles_onlyabsolutes,rs2_A=="CC" | rs2_A=="CT"), 
  ...
  ggtitle("rs2408955/rs112233623 Haplotype Effects") +
  geom_hline(yintercept = baseline,linetype="dashed") +
  geom_text(data = meds, aes(x = con_rs1, y = baseline, label = paste(round(median,3))), size = 3, vjust = -1.8) +
  coord_cartesian(ylim = limits)+
    theme(plot.subtitle = element_text(vjust = 1))

@

<<goshifterLocalZscore, message = FALSE, warning = FALSE, echo = FALSE, fig.height=5, fig.align='center', fig.pos='H', fig.width=8, fig.cap="Local z-scores for enrichment of hematopoietic nucleosome-depleted regions in the set of fine-mapped variants with posterior probability $> 0.10$.", eval = TRUE, cache = FALSE, results="hide">>=
...
for (j in seq(4,4,1)) {
  ...
  CS.PP.gr <- subset(CS.gr,PP > bins[j]) 
  perm[[j]] <- permTest(A=CS.PP.gr, B=peaks.gr, ntimes=1000, alternative="auto", randomize.function=randomizeLocalRegions2, evaluate.function=numOverlaps, force.parallel=FALSE, mc.set.seed=TRUE, mc.cores=2)
}

local <- localZScore(CS.PP.gr, peaks.gr, pt=perm[[4]], window=1500000,step=1000, mc.set.seed=TRUE, mc.cores=2)
plot(local, main="Local Shifted Enrichments for Fine-mapped PP>0.10 Variants")

@

<<fmSecondaryEnrichments, message = FALSE, warning = FALSE, echo = FALSE, fig.height=5, fig.align='center', fig.pos='H', fig.width=8, fig.cap="Local annotation enrichments for fine-mapped variants, excluding all fine-map variants with $R^2>0.80$ to the sentinel variant of any region.", eval = TRUE, cache = FALSE, results="hide">>=

# Exclude LD80 variants from CS
LD80 <- readRDS("../data/Finemap/LD80.rds")
LD80.gr <- GRanges()
for (i in 1:16){
  temp <- as.data.frame(LD80[[i]])[,c("chromosome","sentinel","nonsentinel")]
  temp <- melt(temp,id.vars=c("chromosome"))
  temp$chromosome <- paste0("chr",temp$chromosome)
  temp$end <- temp$value
  temp.gr <- makeGRangesFromDataFrame(temp, keep.extra.columns=F,
                                   ignore.strand=TRUE, 
                                   starts.in.df.are.0based=F,
                                   start.field="value",
                                   end.field="end")
  
  LD80.gr <- mergeRegions(LD80.gr, temp.gr)
}  
...
for (i in seq(1,5,1)) {
  for (j in seq(1,6,1)) {
    CS.PP.gr <- subset(CS.gr,PP > bins[j] & PP <= bins[j+1]) 
    CS.PP.gr <- subtractRegions(CS.PP.gr,LD80.gr)
    perm[[paste0(i,j)]] <- permTest(A=CS.PP.gr, B=gr.list[[i]], ntimes=100, alternative="auto", randomize.function=randomizeLocalRegions2, evaluate.function=numOverlaps, force.parallel=FALSE, mc.set.seed=FALSE, mc.cores=4)
...
ggplot(OR.df,aes(y=OR,x=i,group=j)) + 
  ...
  pretty_plot() +
  scale_y_continuous(expand = c(0.01, 0.0)) +
  scale_x_discrete(labels=c("heme NDRs","coding","promoter","intron","UTR"))+
  scale_fill_manual(values = c(jdb_palette("GrandBudapest2")[c(4)],jdb_palette("Zissou")[1:5]), 
                    name = "top posterior",
                    labels=c("(0.001,0.01]", "(0.01,0.05]", "(0.05,0.1]","(0.1,0.25]","(0.25,0.75]","(0.75,1]")) +
  labs(y="odds ratio") +
...
<<tfRankOrderPlots, message = FALSE, warning = FALSE, echo = FALSE, fig.height=4, fig.align='center', fig.pos='H', fig.width=8, fig.cap="Two subpopulations of CMP and MEP cells were obtained by k-means clustering on ATAC principal components or g-ChromVAR enrichments, respectively. Rank-order plots showing transcription factor binding sites ranked by difference in chromVAR enrichment between the two clusters of (A) CMP and (B) MEP populations", eval = TRUE, cache = FALSE, results="hide">>=
