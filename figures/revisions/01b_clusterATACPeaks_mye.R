...
counts <- counts[,c("HSC", "MPP", "CMP", "GMP-A", "Mono")]
...
mye_countsZ <- ATAC.cpm.log2.z[,c("HSC", "MPP", "CMP", "GMP-A", "Mono")]

dim(mye_countsZ)
pdf(file="plots/mye_cluster_peaks.pdf", width = 3, height = 8)  
par(cex.main=0.8,mar=c(1,1,1,1))
Heatmap(mye_countsZ, col=as.character(jdb_palette("brewer_spectra",type="continuous")),
